...
data class Metal(val albedo: Vec3, val fuzzy: Float) : Material {
    ...
    init {
        if ((0.0f < fuzzy) and (fuzzy > 1.0f))
            throw IllegalArgumentException("fuzzy must be between 0.0f and 1.0f inclusive")
    }
    ...
    override fun scatter(rayIn: Ray, hitRecord: HitRecord): MaterialHitRecord {
        ...
        val shouldScatter = scatterRay.direction.dot(hitRecord.normal) > 0
        return MaterialHitRecord(albedo, scatterRay, shouldScatter)
    }
}